 ...
 */
export const ChatProvider: ForwardRefExoticComponent<ChatProviderProps> =
  forwardRef(({ model, children, chain }, ref) => {
    ...
    const saveSession = async (): Promise<void> => {
      return new Promise(async (resolve, reject) => {
        ...
        if (!node)
          await idb.current.write(session).catch((e) => console.error(e));
    ...
    const fetchResponse = async (prompt: string) => {
      ...
      const payload = {
        chain_type: chain,
        model: "gpt-3.5-turbo",
        messages: session?.chats.slice(-5) || [
